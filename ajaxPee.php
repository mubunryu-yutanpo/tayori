...
if(isset($_POST['d_id']) && !empty($_SESSION['user_id'])){
  ...
  try{
    $dbh = dbConnect();
    //まずは全部のデータを取得する
    $sql = 'SELECT * FROM keep_pee WHERE d_id = :d_id AND user_id = :u_id';
    ...
    if(!empty($result)){
      $sql = 'DELETE FROM keep_pee WHERE d_id = :d_id AND user_id = :u_id';
      $data = array(':d_id'=> $d_id, ':u_id'=> $u_id);
      $stmt = queryPost($dbh,$sql,$data);

    }else{
      //ない場合は登録
      $sql = 'INSERT INTO keep_pee (d_id,user_id,create_date) VALUES (:d_id,:u_id,:date)';
